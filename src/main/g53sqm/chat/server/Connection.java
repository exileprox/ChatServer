...
public class Connection implements Runnable {
	...
	public void run() {
		Thread inThread = new Thread(new Runnable() {
			public void run() {
				...
				running = true;
				sendOverConnection("OK Welcome to the chat server, there are currelty "
						+ serverReference.getNumberOfUsers()
						+ " user(s) online");
				while (running) {
					try {
						line = in.readLine();
	...
	private void stat() {
		...
		case STATE_REGISTERED:
			status += "You are logged im and have sent " + messageCount
					+ " message(s)";
			break;
	...
	private void iden(String message) {
		switch (state) {
		case STATE_REGISTERED:
			sendOverConnection("BAD you are already registerd with username "
					+ username);
			break;
	...
	private void hail(String message) {
		switch (state) {
		case STATE_REGISTERED:
			serverReference.broadcastMessage("Broadcast from " + username
					+ ": " + message);
			messageCount++;
			break;
	...
	private void mesg(String message) {
		...
		case STATE_REGISTERED:
			...
			if (message.contains(" ")) {
				...
				if (serverReference.sendPrivateMessage("PM from " + username
						+ ":" + pm, user)) {
	...
	private void quit() {
		switch (state) {
		case STATE_REGISTERED:
			sendOverConnection("OK thank you for sending " + messageCount
					+ " message(s) with the chat service, goodbye. ");
			break;
		case STATE_UNREGISTERED:
